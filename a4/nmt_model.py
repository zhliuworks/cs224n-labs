...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        source_embed = pack_padded_sequence(source_embed, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(source_embed)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        target_embed = self.model_embeddings.target(target_padded)
        for target_embed_t in target_embed.split(1):
            Ybar_t = torch.cat((target_embed_t.squeeze(0), o_prev), dim=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden = dec_state[0].unsqueeze(-1)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden).squeeze(-1)
