...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = np.dot(prob, outsideVectors)
...
def negSamplingLossAndGradient(
    ...
    outsideVectors[0] *= -1
    prob = sigmoid(np.dot(outsideVectors, centerWordVec))
    loss = np.sum(-np.log(1 - prob))
    gradCenterVec = np.dot(prob, outsideVectors)
    gradOutsideInds = np.outer(prob, centerWordVec)
    gradOutsideInds[0] *= -1
    for i in range(len(indices)):
        gradOutsideVecs[indices[i]] += gradOutsideInds[i]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentCenterInd = word2Ind[currentCenterWord]
    centerWordVec = centerWordVectors[currentCenterInd]
    for outsideWord in outsideWords:
        loss_w, gradCenterVec_w, gradOutsideVecs_w = word2vecLossAndGradient(
            centerWordVec, word2Ind[outsideWord], outsideVectors, dataset)
        loss += loss_w
        gradCenterVecs[currentCenterInd] += gradCenterVec_w
