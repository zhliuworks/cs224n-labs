...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        doc = self.data[idx]
        trun_len = np.random.randint(4, int(self.block_size*7/8)+1)
        doc = doc[:trun_len]  # return doc when len(doc) < trun_len
        doc_len = len(doc)
        mask_len = doc_len * np.random.normal(0.25, 0.1)
        mask_len = int(np.clip(mask_len, 1, doc_len-2))
        mask_idx = np.random.randint(1, doc_len-mask_len)

        prefix = doc[:mask_idx]
        masked = doc[mask_idx:mask_idx+mask_len]
        suffix = doc[mask_idx+mask_len:]
        masked_str = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked + self.MASK_CHAR
        masked_str += (self.block_size - len(masked_str)) * self.PAD_CHAR

        x = torch.tensor([self.stoi[c] for c in masked_str[:-1]], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in masked_str[1:]], dtype=torch.long)
        return x, y
